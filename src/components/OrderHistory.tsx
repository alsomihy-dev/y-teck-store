...
import { Phone, Check, ShieldAlert, Clock, MessageSquare, Truck, CheckCircle2, CreditCard, Package } from 'lucide-react';
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-3xl overflow-hidden shadow-sm text-right mb-8">
      ...
      <div className="bg-slate-950 p-6 text-white flex flex-col sm:flex-row justify-between sm:items-center gap-4">
        ...
        <div className={`text-xs font-bold px-3 py-1.5 rounded-full border w-max ${
           mainOrder.status === 'rejected' ? 'bg-rose-950 border-rose-900 text-rose-400' : 
...
export default function OrderHistory({ orders, notifications = [], onMarkAsRead, onMarkAllAsRead, onBackToHome }: OrderHistoryProps) {
        ...
        {notifications.length > 0 && (
          <div className="bg-white border border-slate-200 rounded-3xl overflow-hidden shadow-sm mb-10">
            <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
              <h3 className="font-bold text-slate-900 text-lg">الإشعارات الحديثة</h3>
